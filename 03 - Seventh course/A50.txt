...
void DeleteClientFromFile(vector <stClientData>& vClientsData, string& AccountNum)
{
	char ConfirmDelete;
	...
	if (CheckIfAccountNumInVector(vClientsData, ClientData, AccountNum))
	{
		...
		{
			MarkClientDataForDelete(vClientsData, AccountNum);

			LoadClientsDataToFile(vClientsData);

			vClientsData = LoadClientsDataFromFile();

			cout << "\n\nClient data has been deleted successfully";
		}
	}
	else
	{
...
int main()
{
	...
	DeleteClientFromFile(vClientsData, AccountNum);
}